...
alias run_tests_gui='pytest tests --ignore=tests/test_tts_services'
alias run_test_audio_fast='pytest -n12 --dist=load tests/test_tts_services/'
alias run_test_debug_logging='pytest --log-cli-level=DEBUG tests/test_components.py -k test_batch_source_1'
alias run_test_audio='pytest --log-cli-level=DEBUG tests/test_tts_services/ -k test_azure'
alias package='./package.sh'
alias coverage_run='coverage run -m pytest' # run tests with coverage
alias coverage_run_gui='coverage run -m pytest --ignore=tests/test_tts_services' # run tests with coverage (GUI tests only)
