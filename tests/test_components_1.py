...
def test_component_source_easy_initial_load_model(qtbot):
    ...
    source_model = config_models.BatchSource(mode=constants.BatchMode.simple, source_field='Pinyin')
...
def test_component_source_easy_text_processing_html_entities(qtbot):
    def build_editor_context_fn(note):
        return config_models.EditorContext(
            note=note, 
            editor=None, 
...
def test_component_source_easy_clipboard_clearing(qtbot):
    ...
    assert source.get_current_text() == 'clipboard text'
    # now, pick "selected"
    source.selection_radio.setChecked(True)
    # check that clipboard was cleared
    assert source.hypertts.anki_utils.clipboard_cleared == True
...
def test_batch_dialog_sound_preview_error(qtbot):
    ...
    def dialog_input_sequence(dialog):
        ...
        assert str(hypertts_instance.anki_utils.last_exception) == 'Service request error for [hello]: Audio not found (voice: Japanese, Male, notfound (ServiceB))'
