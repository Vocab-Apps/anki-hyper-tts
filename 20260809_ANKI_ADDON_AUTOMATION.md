...
- A few repo specifics (exact contents of `.claude/skills/` and `test_utils/`) couldn't be fetched directly due to GitHub anti-bot blocks; recommendations above are based on the confirmed `pytest.ini`, `requirements.txt`, `conftest.py`, `CLAUDE.md`, and `docs/TESTINGPROCEDURE.md`.