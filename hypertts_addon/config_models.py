...
@dataclass
class RemoveAudioConfig:
    """
    how the Remove Audio dialog should process notes. this is transient user
    input, it is not persisted to the addon config.
    """
    # which field to remove audio from, None means every field of the note
    field_name: Optional[str] = None
    # only remove sound tags pointing at audio files generated by HyperTTS
    hypertts_only: bool = True

@dataclass
class RemoveAudioFieldChange:
    """a single field of a single note which would be modified"""
    note_id: int
    field_name: str
    original_text: str
    new_text: str
    ...
    def removed_count(self) -> int:
        return len(self.removed_filenames)
