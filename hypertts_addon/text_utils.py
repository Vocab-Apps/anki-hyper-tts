...
REGEXP_SOUND_TAG = r'\[sound:([^\]]+)\]'

def get_sound_tag_filenames(field_value):
    """return the filename of every sound tag present in this field"""
    return re.findall(REGEXP_SOUND_TAG, field_value)

def is_hypertts_audio_filename(filename):
    return filename.startswith(constants.AUDIO_FILENAME_PREFIX)

# a sound tag is normally separated from the surrounding text by spaces or tabs, which have to
# go with the tag when it's removed. newlines are left alone, they're part of the field's layout
SOUND_TAG_SEPARATOR_CHARS = ' \t'

def remove_sound_tags(field_value, hypertts_only=False):
    """
    remove sound tags from a field, optionally only the ones which point at audio
    generated by HyperTTS. returns (new_field_value, removed_filename_list)

    only the removed tags and the whitespace which separated them from the surrounding text are
    touched, everything else in the field is preserved character for character. the field may
    contain html the user cares about (indented markup, <pre> blocks, deliberate spacing), so
    this must never reformat the parts of the field it isn't removing.
    """
    removed_filenames = []
    result = []
    # index of the first character which hasn't been copied to result yet
    position = 0
    # whether the last thing appended to result was a separator we inserted ourselves
    appended_separator = False

    for match in re.finditer(REGEXP_SOUND_TAG, field_value):
        filename = match.group(1)
        if hypertts_only and not is_hypertts_audio_filename(filename):
            # audio which HyperTTS didn't generate, leave it alone
            continue
        removed_filenames.append(filename)

        # extend the removed span over the separating whitespace, without reaching back into
        # text which was already copied out (two sound tags can share a single space)
        start = match.start()
        while start > position and field_value[start - 1] in SOUND_TAG_SEPARATOR_CHARS:
            start -= 1
        end = match.end()
        while end < len(field_value) and field_value[end] in SOUND_TAG_SEPARATOR_CHARS:
            end += 1

        segment = field_value[position:start]
        if len(segment) > 0:
            result.append(segment)
            appended_separator = False
        # when the tag sat between two pieces of text, the whitespace we just swallowed was
        # separating them, so put a single separator back rather than gluing them together
        whitespace_removed = start < match.start() or end > match.end()
        between_text = start > 0 and end < len(field_value)
        if whitespace_removed and between_text and not appended_separator:
            result.append(' ')
            appended_separator = True
        position = end

    result.append(field_value[position:])
    return ''.join(result), removed_filenames
